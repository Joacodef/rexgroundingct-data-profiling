...
% ==============================================================================
\begin{frame}{Canonical 3D RAS Centroids \& Spatial Taxonomy}
    \centering
    \scriptsize
    \setlength{\tabcolsep}{7pt}
    ...
    \note{
        This table details the canonical 3D RAS spatial coordinate centroids and our 4-tier spatial density prior taxonomy across all 14 categories.

        Findings are grouped into four spatial taxonomies based on 3D probability density maps:
        First, Apical Dominant, like Emphysema, which concentrates in upper lung zones with an Inferior-Superior centroid of 0.667.
        Second, Basal or Dependent, like Atelectasis and Pleural Effusion, which cluster in lower posterior zones.
        Third, Hilar or Peribronchial, like Bronchial Wall Thickening and Bronchiectasis, which track central airway structures.
        And fourth, Isotropic or Parenchymal, like Nodules and Ground-glass opacity, which distribute broadly across lung fields.
    }
\end{frame}
...
% ==============================================================================
\begin{frame}{HU Radiodensity \& Contrast Deltas}
    \begin{columns}[c]
        \begin{column}{0.5\textwidth}
            \centering
    ...
    \note{
        Here we visualize the HU radiodensity profiles.

        The left plot shows violin distributions of HU values for each of the 14 categories, highlighting the wide spectrum from low-density air-trapping pathologies to high-density soft-tissue opacities.

        The right plot quantifies contrast deltas between pathology regions and their surrounding parenchymal boundaries, demonstrating clear intensity gradients that can be leveraged during model windowing.
    }
\end{frame}
...
\begin{frame}{Calibrated Actionable Modeling Directives}
    ...
    \note{
        To conclude, our Phase 1 profiling yields five mandatory calibrated directives for downstream baseline evaluation and fine-tuning:
        ...
        Fourth, apply component size threshold pruning to remove false-positive noise.
        And fifth, enforce 4D Back-Reorientation to ensure perfect spatial alignment with ground-truth NIfTI metadata.
    }
\end{frame}
