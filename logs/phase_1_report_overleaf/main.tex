...
\begin{enumerate}
    ...
    \item \textbf{Experiment 002 (\texttt{exp\_002\_nlp\_prompt\_syntax})}: Free-text NLP prompt syntax shift (Cases 1--50 vs.\ 51--200), BPE subword tokenization dynamics (+34.2\% expansion), spatial preposition locators (64.39\%), and diagnostic hedging pruning (0.38\%).
    ...
    \item \textbf{Experiment 005 (\texttt{exp\_005\_morphology\_noise\_pruning})}: 3D connected-component morphology across 33,058 blobs, sphericity index ($S$), surface area-to-volume ($SA/V$) ratios, volume quantiles, and recommended noise pruning thresholds (\texttt{recommended\_min\_size\_voxels}).
\end{enumerate}
...
\toprule
\textbf{Split} & \textbf{Scans} & \textbf{Findings} & \makecell{\textbf{Findings}\\\textbf{/Scan}} & \makecell{\textbf{Mean Inst}\\\textbf{/Find}} & \textbf{Std Inst} & \textbf{Max Inst} \\
\midrule
...
\begin{tabular}{lcccc}
\toprule
\textbf{Metric / Parameter} & \makecell{\textbf{Cases 1--50}\\\textbf{(Paper)}} & \makecell{\textbf{Cases 51--200}\\\textbf{(MICCAI)}} & \makecell{\textbf{Shift /}\\\textbf{Factor}} & \textbf{Combined Val} \\
\midrule
...
\begin{tabular}{lcccccc}
\toprule
\textbf{Cat Code} & \textbf{Category Name} & \textbf{Mean HU} & \textbf{P5 HU} & \textbf{P95 HU} & \makecell{\textbf{Contrast}\\\textbf{$\Delta\text{HU}$}} & \makecell{\textbf{Rec Window}\\\textbf{[Min, Max]}} \\
\midrule
...
\texttt{2h} & Other focal & -156.62 & -915.0 & +201.0 & -16.21 & [-156.62, +201.0] \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\begin{tabular}{lcccccc}
\toprule
\textbf{Cat Code} & \textbf{Category Name} & \textbf{Blobs} & \makecell{\textbf{Blobs/}\\\textbf{Find}} & \makecell{\textbf{Sphericity}\\\textbf{($S$)}} & \makecell{\textbf{SA/V}\\\textbf{(mm$^{-1}$)}} & \makecell{\textbf{Rec Min Size}\\\textbf{(voxels)}} \\
\midrule
