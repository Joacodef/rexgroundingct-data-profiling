...
To eliminate confusion across dataset metrics, we define the 4-tier dataset indexing structure:
\begin{itemize}
    \item \textbf{Physical NIfTI Scans (3,078 volumes)}: The ground-truth image repository containing 2,578 Training scans, 200 Validation scans, and 300 Test scans.
    \item \textbf{Dataset Record Entries (3,192 entries)}: The primary indexing structures in \texttt{dataset.json} (comprising 2,992 Training entries and 200 Validation entries).
    \item \textbf{Target Findings (3,758 findings)}: Discrete 3D segmentation target masks (3,192 in Train, 566 in Val).
    \item \textbf{Radiology Report Prompts (8,650 prompts)}: Free-text sentence queries (averaging 2.3 prompt variations per finding target).
\end{itemize}
...
\begin{table}[H]
\centering
\caption{ReXGroundingCT Master Dataset Parameters, Hardware Acquisition Specs, and Cohort Metadata.}
...
Acquisition Window \& Modality & February 2016 -- November 2022; Non-contrast 3D Thoracic CT \\
...
Candidate Finding Exclusion Audit & 10,012 initial candidate findings $\rightarrow$ 8,028 eligible (1,984 excluded) \\
...
\begin{enumerate}
    \item \textbf{Annotation Disparity \& Instance Fragmentation (Q1)}: What is the quantitative gap between partially labeled training scans ($\sim 1.07$ findings/scan, $1.948$ instances/finding) and exhaustively labeled validation scans ($\sim 2.83$ findings/scan, $3.714$ instances/finding)?
    \item \textbf{Text Prompt Shift \& Syntactic Complexity (Q2)}: How does the free-text prompt structure shift from short paper queries (mean 11 words) to long multi-clause report sentences (up to 38 words, $+12.4\%$ commas, $-45.2\%$ TTR), multi-finding compound prompts ($18.77\%$), hedging ($0.38\%$), and spatial prepositions ($72.53\%$)?
...
\midrule
\texttt{exp\_001} & Mask Disparity \& Sparsity & 2,992 Train vs 200 Val entries & Quantified partial (1.07/entry) vs exhaustive (2.83/entry) gap \\
\texttt{exp\_002} & Prompt Text Shift & Paper split vs full MICCAI split & Measured prompt length spike (+9.3\%) and TTR drop (-45.2\%) \\
...
Quantitative analysis of the master dataset annotations demonstrates a fundamental structural disparity between training and validation annotation protocols. As detailed in Table~\ref{tab:split_disparity}, training record entries contain an average of \textbf{1.07 findings per entry} (corresponding to \textbf{1.24 findings per physical NIfTI scan} across 2,578 physical training volumes) and \textbf{1.948 labeled entity instances per finding} (totaling $\sim 7,633$ annotated target lesion instances overall), adhering to a \textit{partial annotation protocol} where annotators selectively labeled primary lesion instances per scan. In contrast, validation record entries contain an average of \textbf{2.83 findings per entry} and \textbf{3.714 instances per finding} (reaching up to \textbf{36 instances} per finding for diffuse ground-glass opacities), reflecting an \textit{exhaustive annotation protocol}.

It is crucial to distinguish between \textit{Annotated Finding Targets} ($3,758$ total findings in Table~\ref{tab:split_disparity}) and \textit{3D Connected Component Blobs} ($27,136$ total blobs in Table~\ref{tab:category_prevalence} and Table~\ref{tab:morphology}). In 3D medical image segmentation, a single annotated finding target mask (e.g., multifocal ground-glass opacities or emphysema) frequently comprises multiple spatially disconnected 3D lesion components. Topological extraction via 3D connected-component labeling (\texttt{scipy.ndimage.label}) yields $27,136$ discrete 3D blobs across the dataset (averaging $3.364$ connected blobs per pathology-scan occurrence across the 8,067 category-scan instances in Table~\ref{tab:category_prevalence}).

Furthermore, summing the \textit{Scans with Pathology} column across all 14 categories in Table~\ref{tab:category_prevalence} yields $8,067$ category-scan instances. This figure exceeds the $3,758$ annotated target findings in Table~\ref{tab:split_disparity} because CT scans in ReXGroundingCT frequently exhibit multi-label pathology co-occurrences (e.g., a single scan simultaneously exhibiting ground-glass opacity, atelectasis, and emphysema). Under partial annotation in Train, annotators selectively labeled a primary subset of findings ($3,758$ total target finding masks), whereas category-level profiling evaluates all multi-label pathology occurrences across scans ($8,067$ total category-scan instances).

This structural disparity has profound implications for supervised model training. Standard supervised segmentation losses (such as Binary Cross-Entropy or Dice Loss) assume that unannotated background voxels represent true negative healthy tissue. Under partial annotation, however, unannotated background regions in the training set harbor true-positive lesions. Applying standard BCE/Dice losses directly penalizes predictions on unannotated true lesions, inducing severe \textit{Instance Suppression Bias} during gradient backpropagation.
...
\midrule
\textbf{Training Split} & 2,992 & 3,192 & \textbf{1.07} & \textbf{1.948 $\pm$ 1.25} & 11 & Partial \\
\textbf{Validation Split} & 200 & 566 & \textbf{2.83} & \textbf{3.714 $\pm$ 3.82} & \textbf{36} & Exhaustive \\
\textbf{Overall Dataset} & 3,192 & 3,758 & \textbf{1.18} & \textbf{2.031 $\pm$ 1.63} & \textbf{36} & Mixed \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{itemize}
    \item \textbf{Word Count Increase}: Mean prompt length increased from \textbf{10.98 words} in paper cases 1--50 to \textbf{12.00 words} in cases 51--200 ($+9.3\%$), with maximum sentence length spiking to \textbf{38 words}.
    \item \textbf{Lexical Diversity Drop}: Type-Token Ratio (TTR) dropped from \textbf{0.1544} down to \textbf{0.0846} (a \textbf{--45.2\% reduction}), indicating highly repetitive clinical vocabulary across complex report descriptions.
...
\subsection{Canonical RAS Grid Resampling}
To establish spatial priors independent of scanner orientation or slice thickness, all ground-truth masks were reoriented to canonical Right-Anterior-Superior (RAS) spatial axes and resampled onto a normalized $128 \times 128 \times 128$ isotropic reference grid. Normalized 3D probability density maps $P(\mathbf{x}' \in \text{mask} \mid c)$ and 2D Average Intensity Projections (AIP) were computed per category.

\subsection{Category Spatial Priors \& Centroid Analysis}
Table~\ref{tab:centroids} lists the spatial centroids $(R\text{-}L, A\text{-}P, I\text{-}S)$ normalized to $[0, 1]^3$. Figure~\ref{fig:spatial_density} displays representative 2D AIP projections:
\begin{itemize}
...
\begin{table}[H]
\centering
\caption{Canonical RAS space normalized spatial centroids $(R\text{-}L, A\text{-}P, I\text{-}S)$ for Train and Val splits.}
\label{tab:centroids}
\small
\resizebox{\linewidth}{!}{%
\begin{tabular}{lccl}
...
Emphysema & (0.494, 0.556, 0.667) & (0.462, 0.554, 0.789) & \textbf{Upper lobe apical dominance} ($Z_{\text{IS}} > 0.65$) \\
...
Atelectasis / consolidation & (0.495, 0.565, 0.491) & (0.519, 0.560, 0.416) & \textbf{Basal \& dependent lung preference} ($Z_{\text{IS}} < 0.50$) \\
Ground-glass opacity & (0.488, 0.591, 0.519) & (0.450, 0.622, 0.442) & Multifocal peripheral \& lower lobe spread \\
Pulmonary nodules / masses & (0.458, 0.546, 0.556) & (0.454, 0.551, 0.554) & \textbf{Isotropic parenchymal distribution} ($\sim 0.50$) \\
Pleural effusion / thickening & (0.463, 0.684, 0.484) & (0.457, 0.668, 0.431) & \textbf{Posterior-basal boundary} ($Y_{\text{AP}} > 0.65$) \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Positive-Unlabeled (PU) SPOCO Fine-Tuning}: Incorporate class-weighted PU-SPOCO loss with float32 upcasting and L2 gradient clipping to mitigate background suppression bias on unannotated training findings (justified by the dual-protocol partial annotation lineage in Train where $1.07$ findings/scan are annotated vs $2.83$ findings/scan in Val).
