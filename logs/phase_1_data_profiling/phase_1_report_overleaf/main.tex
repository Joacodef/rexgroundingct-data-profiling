...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Distribution of positive finding entities per CT scan across Train ($N=2,992$) and Val ($N=200$) splits.}
    \label{fig:findings_per_scan}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp001_entity_counts_hist.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig/exp008_relative_volume_boxplot.png}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Prevalence distribution of positive findings across the 14 official categories in Train vs. Val splits.}
    \label{fig:category_freq}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp001_finding_prompts_per_scan_distribution.png}
...
To establish spatial priors independent of scanner orientation or slice thickness, all ground-truth masks were reoriented to canonical Right-Anterior-Superior (RAS) spatial axes and resampled onto a normalized $128 \times 128 \times 128$ isotropic reference grid. Because ground-truth NIfTI segmentation headers contain an identity affine metadata format, each segmentation mask array inherits the physical affine matrix of its parent CT scan (\texttt{nib.Nifti1Image(mask\_data, img\_nii.affine)}) prior to invoking canonical reorientation (\texttt{nib.as\_closest\_canonical}). Normalized 3D probability density maps $P(\mathbf{x}' \in \text{mask} \mid c)$ and 2D Average Intensity Projections (AIP) were computed per category across Train and Val splits.

\subsection{Category Spatial Priors \& Centroid Analysis}
Table~\ref{tab:centroids} lists the spatial centroids $(R\text{-}L, A\text{-}P, I\text{-}S)$ normalized to $[0, 1]^3$ alongside inter-split normalized centroid Euclidean distances ($\Delta d$) across all 14 official categories. To prevent small-sample validation Poisson noise ($1/\sqrt{N_{\text{val}}}$) from distorting population priors, Figure~\ref{fig:spatial_density} showcases canonical population spatial probability density maps computed over the full training set ($N=7,687$ finding masks across 2,992 scans). All 14 pathologies are categorized into a 4-tier anatomical spatial prior taxonomy:
\begin{itemize}
    \item \textbf{Apical Dominance (Type I)}: \textbf{Emphysema (\texttt{1c})} demonstrates strong upper-lobe apical concentration ($Z_{\text{IS}} = 0.667$), reflecting centrilobular emphysematous parenchymal destruction in superior apical lung segments.
    \item \textbf{Basal / Dependent Preference (Type II)}: \textbf{Atelectasis / Consolidation (\texttt{2b})} ($Z_{\text{IS}} = 0.491$), \textbf{Pleural Effusion / Thickening (\texttt{2e})} ($Y_{\text{AP}} = 0.684, Z_{\text{IS}} = 0.484$), and \textbf{Honeycombing (\texttt{2f})} ($Z_{\text{IS}} = 0.498$) exhibit strong posterior-basal dependent lung localization, consistent with gravitational fluid collection and basal fibrotic changes.
    \item \textbf{Hilar / Peribronchial Concentration (Type III)}: \textbf{Bronchial Wall Thickening (\texttt{1a})} ($0.490, 0.559, 0.541$) and \textbf{Bronchiectasis (\texttt{1b})} ($0.488, 0.549, 0.545$) are concentrated around the central hilar and bronchovascular bundles.
    \item \textbf{Isotropic / Parenchymal Distribution (Type IV)}: \textbf{Pulmonary Nodules / Masses (\texttt{2d})} ($0.458, 0.546, 0.556$), \textbf{Ground-Glass Opacity (\texttt{2c})}, and \textbf{Micronodules (\texttt{1e})} display isotropic, multifocal spatial distributions throughout the lung parenchyma.
    \item \textbf{Quantitative Inter-Split Alignment Rationale}: For high-prevalence categories ($N_{\text{val}} \ge 69$, e.g., Nodules \texttt{2d} $N_{\text{val}}=132$, Linear opacities \texttt{2a} $N_{\text{val}}=69$), inter-split centroid distances are extremely tight ($\Delta d = 0.0067$ and $\Delta d = 0.0223$, respectively). For low-prevalence validation categories ($N_{\text{val}} \le 6$, e.g., Pneumothorax \texttt{2g} $N_{\text{val}}=1, \Delta d = 0.2243$, Septal thickening \texttt{1d} $N_{\text{val}}=6, \Delta d = 0.1242$), centroid deltas reflect single-case sampling variance rather than anatomical shift.
\end{itemize}

\begin{table}[H]
\centering
\caption{Canonical RAS space normalized spatial centroids $(R\text{-}L, A\text{-}P, I\text{-}S)$, sample mask counts ($N_{\text{train}}, N_{\text{val}}$), inter-split normalized centroid Euclidean distances ($\Delta d$), and Anatomical Spatial Prior Taxonomy across all 14 official categories.}
\label{tab:centroids}
\small
\resizebox{\linewidth}{!}{%
\begin{tabular}{lccccccl}
\toprule
\textbf{Category Name} & \textbf{Train $N$} & \textbf{Train Centroid} & \textbf{Val $N$} & \textbf{Val Centroid} & \textbf{$\Delta d$} & \textbf{Spatial Prior Taxonomy} \\
\midrule
Bronchial wall thickening (\texttt{1a}) & 236 & (0.508, 0.439, 0.541) & 3 & (0.495, 0.491, 0.478) & 0.0825 & Hilar / Peribronchial \\
Bronchiectasis (\texttt{1b}) & 282 & (0.510, 0.449, 0.545) & 11 & (0.471, 0.489, 0.548) & 0.0563 & Hilar / Peribronchial \\
Emphysema (\texttt{1c}) & 446 & (0.504, 0.442, 0.667) & 17 & (0.466, 0.533, 0.789) & 0.1566 & \textbf{Apical Dominance} ($Z_{\text{IS}} > 0.65$) \\
Septal thickening (\texttt{1d}) & 194 & (0.519, 0.420, 0.563) & 6 & (0.519, 0.474, 0.469) & 0.1092 & Isotropic / Parenchymal \\
Micronodules (\texttt{1e}) & 314 & (0.517, 0.422, 0.537) & 11 & (0.468, 0.499, 0.580) & 0.1007 & Isotropic / Parenchymal \\
Other non-focal (\texttt{1f}) & 150 & (0.501, 0.426, 0.545) & 4 & (0.404, 0.488, 0.479) & 0.1327 & Isotropic / Parenchymal \\
Linear opacities (\texttt{2a}) & 1120 & (0.474, 0.500, 0.535) & 69 & (0.563, 0.492, 0.537) & 0.0886 & Isotropic / Parenchymal \\
Atelectasis / consolidation (\texttt{2b}) & 1367 & (0.503, 0.433, 0.491) & 49 & (0.531, 0.507, 0.416) & 0.1084 & \textbf{Basal \& Dependent} ($Z_{\text{IS}} < 0.50$) \\
Ground-glass opacity (\texttt{2c}) & 1507 & (0.510, 0.407, 0.519) & 60 & (0.497, 0.560, 0.442) & 0.1711 & Isotropic / Parenchymal \\
Pulmonary nodules / masses (\texttt{2d}) & 1743 & (0.540, 0.452, 0.556) & 132 & (0.466, 0.521, 0.554) & 0.1012 & \textbf{Isotropic Parenchymal} ($\sim 0.50$) \\
Pleural effusion / thickening (\texttt{2e}) & 237 & (0.535, 0.314, 0.484) & 11 & (0.461, 0.539, 0.431) & 0.2426 & \textbf{Posterior-Basal} ($Y_{\text{AP}} > 0.65$) \\
Honeycombing (\texttt{2f}) & 16 & (0.457, 0.482, 0.498) & 0 & N/A & N/A & Basal \& Dependent \\
Pneumothorax (\texttt{2g}) & 18 & (0.497, 0.516, 0.522) & 1 & (0.304, 0.443, 0.622) & 0.2293 & Isotropic / Peripheral \\
Other focal (\texttt{2h}) & 57 & (0.521, 0.442, 0.536) & 7 & (0.513, 0.488, 0.578) & \textbf{0.0633} & Isotropic / Parenchymal \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp003_average_mask_1c_emphysema.png}
    \caption{Emphysema (\texttt{1c}): Apical Dominant Prior.}
    \label{fig:aip_emphysema}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp003_average_mask_2e_pleural_effusion___thickening.png}
    \caption{Pleural Effusion (\texttt{2e}): Posterior-Basal Dependent Prior.}
    \label{fig:aip_effusion}
\end{subfigure}
\\ \vspace{0.2cm}
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp003_average_mask_1a_bronchial_wall_thickening.png}
    \caption{Bronchial Wall Thickening (\texttt{1a}): Hilar / Peribronchial Prior.}
    \label{fig:aip_bronchial}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp003_average_mask_2d_pulmonary_nodules___masses.png}
    \caption{Pulmonary Nodules / Masses (\texttt{2d}): Isotropic Parenchymal Prior.}
    \label{fig:aip_nodules}
\end{subfigure}
\caption{Canonical RAS ($128 \times 128 \times 128$) Population Spatial Probability Density Maps ($N=7,687$) illustrating the 4-tier Anatomical Spatial Prior Taxonomy across representative pathology categories from the full training set.}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{CT attenuation distribution (HU) inside ground-truth mask regions versus healthy parenchymal tissue.}
    \label{fig:hu_violin}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
...
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp005_cooccurrence_matrix.png}
    \caption{Multi-finding co-occurrence probability matrix $P(c_i \mid c_j)$ across CT scans.}
    \label{fig:cooccurrence}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{fig/exp006_inter_class_iou_matrix.png}
